...
def bert_batch_preprocessing(batch):
    ...
    y_mask_b, y_emotions_b, y_causes_b, mask_b = pad_docs(doc_len_b, y_emotions_b, y_causes_b)
    ...
    return np.array(doc_len_b), np.array(adj_b), \
           ...
           bert_token_b, bert_segment_b, bert_masks_b, bert_clause_b, mask_b


def pad_docs(doc_len_b, y_emotions_b, y_causes_b):
    max_doc_len = max(doc_len_b)

    mask_b, y_mask_b, y_emotions_b_, y_causes_b_ = [], [], [], []
    for y_emotions, y_causes in zip(y_emotions_b, y_causes_b):
        ...
        mask = list(map(lambda x: 1 if x == -1 else 0, y_emotions_))
    ...
    return y_mask_b, y_emotions_b_, y_causes_b_, mask_b
