...
def eval_consistency(emo_all, cau_all, doc_couples_pred_all):
    tmp_num = {'e': 0, 'c': 0}
    tmp_den_p = {'e': 0, 'c': 0}
    tmp_den_r = {'e': 0, 'c': 0}

    for emos, caus, doc_couples_pred in zip(emo_all, cau_all, doc_couples_pred_all):
        doc_couples_pred = set([','.join(list(map(lambda x: str(x), doc_couple))) for doc_couple in doc_couples_pred])
        doc_emos = set([doc_couple.split(',')[0] for doc_couple in doc_couples_pred])
        doc_emos_pred = set([str(emo) for emo in emos]) 
        tmp_num['e'] += len(doc_emos & doc_emos_pred)
        tmp_den_p['e'] += len(doc_emos_pred)
        tmp_den_r['e'] += len(doc_emos)

        doc_caus = set([doc_couple.split(',')[1] for doc_couple in doc_couples_pred]) 
        doc_caus_pred = set([str(cau) for cau in caus])
        tmp_num['c'] += len(doc_caus & doc_caus_pred)
        tmp_den_p['c'] += len(doc_caus_pred)
        tmp_den_r['c'] += len(doc_caus)
